...
def attention_gate(g, x, num_filters):
    ...
    # Getting the gating signal to the same number of
    # filters as the inter_shape
    phi_g = tf.keras.layers.Conv2D(filters=num_filters,
                                   kernel_size=1,
                                   strides=1,
                                   padding='same')(g)
...
def decoder_block(x, concat, num_filters, size, dropout, batch_norm):
    ...
    de_conv = res_conv_block(u, num_filters, size, dropout,
                             batch_norm)

    return de_conv


def create_att_res_u_net():
    # size of images
    IMG_WIDTH = 256
    IMG_HEIGHT = 256
    IMG_CHANNELS = 3
    ...
    model_name = 'new_model_Att_Res_u_net'
